...
export default function AdminProfilePage() {
  ...
  const [profile, setProfile] = useState({
    ...
    address: "123 Admin Plaza, Suite 900",
    city: "New York",
  ...
  useEffect(() => {
    if (session?.user) {
      setProfile(prev => ({
        ...
        image: session.user.image || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&h=400&fit=crop"
      }));
    } else {
      // Fallback details
      setProfile(prev => ({
    ...
    <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-5">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">Admin Profile</h1>
          <p className="text-gray-500 text-sm">Manage your system credentials, alerts, and billing parameters.</p>
        </div>
      </div>
